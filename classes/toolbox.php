...
class toolbox {
    ...
    static public function get_csswww() {
        ...
        if (!self::lte_ie9()) {
            if (\right_to_left()) {
                $moodlecss = 'legend-rtl.css';
            } else {
                $moodlecss = 'legend.css';
            }

            $syscontext = \context_system::instance();
            $itemid = \theme_get_revision();
            $url = \moodle_url::make_file_url("$CFG->wwwroot/pluginfile.php", "/$syscontext->id/theme_legend/style/$itemid/$moodlecss");
            $url = preg_replace('|^https?://|i', '//', $url->out(false));
            return '<link rel="stylesheet" href="'.$url.'">';
        } else {
            if (\right_to_left()) {
                $moodlecssone = 'legend-rtl_ie9-blessed1.css';
                $moodlecsstwo = 'legend-rtl_ie9.css';
            } else {
                $moodlecssone = 'legend_ie9-blessed1.css';
                $moodlecsstwo = 'legend_ie9.css';
            }

            $syscontext = \context_system::instance();
            $itemid = \theme_get_revision();
            $urlone = \moodle_url::make_file_url("$CFG->wwwroot/pluginfile.php", "/$syscontext->id/theme_legend/style/$itemid/$moodlecssone");
            $urlone = preg_replace('|^https?://|i', '//', $urlone->out(false));
            $urltwo = \moodle_url::make_file_url("$CFG->wwwroot/pluginfile.php", "/$syscontext->id/theme_legend/style/$itemid/$moodlecsstwo");
    ...
    static protected function get_theme_config() {
        if (empty(self::$theme)) {
            self::$theme = \theme_config::load('legend');
        }
        return self::$theme;
    }
    ...
    /**
     * Finds the given include file in the theme.  If it does not exist for the legend child theme then the parent is checked.
    ...
    static public function get_include_file($filename) {
        ...
        $filename .= '.php';
        // Check only if a child of 'legend' to prevent conflicts with other themes using the 'includes' folder....
        if (in_array('legend', $PAGE->theme->parents)) {
        ...
        // Check legend.
        if (file_exists("$CFG->dirroot/theme/legend/layout/includes/$filename")) {
            return "$CFG->dirroot/theme/legend/layout/includes/$filename";
        } else if (!empty($CFG->themedir) and file_exists("$CFG->themedir/legend/layout/includes/$filename")) {
            return "$CFG->themedir/legend/layout/includes/$filename";
        } else {
     ...
     * If it does not exist for the legend child theme then the parent is checked.
    ...
    static public function include_folder($folder) {
        ...
        $themedir = $PAGE->theme->dir;
        $thefolder = false;
        // Check only if a child of 'legend'.
        if (in_array('legend', $PAGE->theme->parents)) {
            $themename = $PAGE->theme->name;
        ...
        if (!$thefolder) {
            // Check legend.
            if (file_exists("$CFG->dirroot/theme/legend/$folder")) {
                $thefolder = "$CFG->dirroot/theme/legend/$folder";
            } else if (!empty($CFG->themedir) and file_exists("$CFG->themedir/legend/$folder")) {
                $thefolder = "$CFG->themedir/legend/$folder";
            } else {
    ...
    static public function render_slide_controls($left) {
        ...
        $prev = '<a class="left carousel-control" href="#legendCarousel" data-slide="prev"><i class="fa fa-chevron-circle-' . $faleft . '"></i></a>';
        $next = '<a class="right carousel-control" href="#legendCarousel" data-slide="next"><i class="fa fa-chevron-circle-' . $faright . '"></i></a>';
    ...
    static public function get_nav_links($course, $sections, $sectionno) {
        ...
        while ($back > 0 and empty($links['previous'])) {
           if ($canviewhidden || $sections[$back]->uservisible) {
                ...
                $previouslink .= \get_string('previoussection', 'theme_legend');
        ...
        while ($forward <= $course->numsections and empty($links['next'])) {
            if ($canviewhidden || $sections[$forward]->uservisible) {
                ...
                $nextlink .= \get_string('nextsection', 'theme_legend');
    ...
    static protected function check_colours_switch() {
        $colours = \optional_param('legendcolours', null, PARAM_ALPHANUM);
        if (in_array($colours, array('default', 'alternative1', 'alternative2', 'alternative3', 'alternative4'))) {
            \set_user_preference('theme_legend_colours', $colours);
        }
    }
    ...
    static public function initialise_colourswitcher(\moodle_page $page) {
        ...
        \user_preference_allow_ajax_update('theme_legend_colours', PARAM_ALPHANUM);
        $page->requires->yui_module(
                'moodle-theme_legend-coloursswitcher', 'M.theme_legend.initColoursSwitcher',
    ...
    static public function get_colours($default = 'default') {
        $preference = \get_user_preferences('theme_legend_colours', $default);
    ...
    static public function set_font($css, $type, $fontname) {
        ...
        if (empty($fontname)) {
            ...
            $facereplacement = '';
        } else if (\theme_legend\toolbox::get_setting('fontselect') === '3') {
    ...
    static public function set_logowidth($css, $logowidth) {
